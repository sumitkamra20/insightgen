...
gcloud run deploy insightgen-api \
  ...
  --memory=2Gi \
  --cpu=1 \
  --set-env-vars="STORAGE_MODE=gcs,GCS_BUCKET=$GCS_BUCKET,PORT=8090,OPENAI_API=$OPENAI_API,OPENAI_OBSERVATIONS_MODEL=${OPENAI_OBSERVATIONS_MODEL:-gpt-4o},OPENAI_HEADLINES_MODEL=${OPENAI_HEADLINES_MODEL:-gpt-4o}" \
  --timeout=3600s \
